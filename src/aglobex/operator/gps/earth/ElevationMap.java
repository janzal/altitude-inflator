...
public class ElevationMap {
    ...
    /**
     * Will create new elevation map from given repository and in maximum depth.
     * @param repositoryPath to be used
    ...
    private ElevationTile fetchTile(int idxLatitude, int idxLongitude, byte depth) {
        for(;depth>=0;depth--,idxLatitude/=2, idxLongitude/=2) {
            ...
            try {
                tile = new ElevationTile(file, idxLatitude, idxLongitude, depth);
            }catch(IOException e) {
    ...
    private File getTileFile(int idxLatitude, int idxLongitude, byte depth) {
        assert(depth<14);
        return new File(String.format(repositoryPath+"/"+DATASET_DIRECTORY
                +"/%02d/%04d_%04d.%s", depth, idxLongitude, idxLatitude, DATA_EXTENSION));
    }
    ...
    public void buildElevationMapTile(int idxLatitude, int idxLongitude, byte depth, 
        ...
        ElevationTile target = new ElevationTile(data, idxLatitude, idxLongitude, depth);
